# Project Roadmap and TODOs

## Frontend Improvements
- Implement a dark mode toggle.
- Add a more robust routing system for nested component views.
- Enhance the search overlay with keyboard shortcuts.
- Create a consistent design system for all demo components.

## Backend Enhancements
- Implement a more dynamic registry that reads from a config file instead of hardcoded lists.
- Add more complex system utilities (e.g., process monitoring, network diagnostics).
- Improve error logging and reporting via the Tauri log system.
- Implement a caching layer for utility source code retrieval.

## Testing and Quality
- Increase unit test coverage for the registry logic.
- Add integration tests for Tauri commands.
- Implement automated build checks in the CI pipeline.
- Perform a full audit of file system access for security vulnerabilities.

## Documentation
- Expand the deployment guide with platform-specific tips.
- Add a contribution guide for external developers.
- Document the API contract between frontend and backend.
